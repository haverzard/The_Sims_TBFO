...
class Player:
	...
	def doing(self, aksi):
		choices = {
			...
			"Bersosialisasi ke Kafe": (lambda x: x.sign(-5,-10,15)),
			"Bermain Media Sosial": (lambda x: x.sign(0,-10,10)),
			"Bermain Komputer": (lambda x: x.sign(0,-10,15)),
			"Mandi": (lambda x: x.sign(15,-5,0)),
			"Cuci Tangan": (lambda x: x.sign(5,0,0)),
			"Mendengarkan Musik di Radio": (lambda x: x.sign(0,-5,10)),
			"Membaca Koran": (lambda x: x.sign(0,-5,5)),
			"Membaca Novel": (lambda x: x.sign(0,-5,10))
		}
